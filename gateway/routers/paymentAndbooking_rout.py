...
@router.post(
    "/all-bookings",
    response_model=_schemas.AllBookingsGatewayResponse,
    tags=["Bookings"]
)
async def get_all_bookings_gateway(
    request_data: _schemas.BookingListGatewayRequest,
    user: dict = Depends(verify_token),
):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return build_response(
                data=[],
                request_id=request_data.header.requestId,
                message=response.json().get("detail", "Failed to fetch bookings"),
                code=str(response.status_code),
            )
